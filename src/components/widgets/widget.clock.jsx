...
export default function WidgetClock(identifier) {
    ...
    useEffect(() => {
        // analog clock - set degrees of each hand
        setHourHand(datetime.getHours() * 30 + datetime.getMinutes() / 2)
        setMinuteHand(datetime.getMinutes() * 6 + datetime.getSeconds() / 10)
        setSecondHand(datetime.getSeconds() * 6)

        // progressbar - update day percentage
        setDayPercentage(((datetime.getHours()*4.17) + (datetime.getMinutes()/60 * 4.17)).toFixed(2))
    }, [datetime]);

    const saveActiveTab = (k) => {
                ...
                <div id="clock" className="clock-content">
                    ...
                    <div
                        ...
                        className={
                            ...
                            (time.hour === 0 ? "" : "transition-effect")
                        }
                        style={{transform: "rotateZ(" + time.hour + "deg)"}}></div>
                    <div
                        id="minutes-indicator"
                        className={
                            "indicator minutes-indicator " +
                            (time.minute === 0 ? "" : "transition-effect")
                        }
                        style={{transform: "rotateZ(" + time.minute + "deg)"}}></div>
                    <div
                        id="seconds-indicator"
                        className={
                            "indicator seconds-indicator " +
                            (time.second === 0 ? "" : "transition-effect")
                        }
                        style={{transform: "rotateZ(" + time.second + "deg)"}}></div>
